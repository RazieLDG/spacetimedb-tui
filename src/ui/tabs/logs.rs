...
fn render_toolbar(area: Rect, buf: &mut Buffer, app: &AppState, paused: bool) {
    ...
    let left = Line::from(vec![
        ...
        Span::styled(format!("db: {db} "), Style::default().fg(accent)),
        Span::styled("│ ", Style::default().fg(Color::Rgb(50, 50, 60))),
        Span::styled(
...
fn render_log_lines(area: Rect, buf: &mut Buffer, app: &AppState, paused: bool) {
    ...
    for (row, entry) in filtered.iter().enumerate().skip(scroll).take(visible_h) {
        ...
        let ts_span = Span::styled(format!("{ts} "), Style::default().fg(FG_TS).bg(bg));
        ...
        let target_span = if let Some(ref t) = entry.target {
            Span::styled(format!("[{t}] "), Style::default().fg(fg_muted).bg(bg))
        } else {
            Span::raw("")
        };

        let msg_span = Span::styled(entry.message.clone(), Style::default().fg(FG_MSG).bg(bg));
    ...
    if total > visible_h {
        ...
        let indicator = format!(
            " {}/{} ({}%) ",
            scroll + visible_h.min(total - scroll),
            total,
            pct
        );
